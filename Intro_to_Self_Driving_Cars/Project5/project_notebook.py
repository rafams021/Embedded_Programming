...
def create_gScore(self):
    ...
    for i, j in enumerate(self.map.intersections):
        gScore[i] = float("inf")
    gScore[self.start] = 0
...
def get_current_node(self):
    ...
    for node in self.openSet:
        if (c_node == None):
            c_node = node
            c_min = self.calculate_fscore(node)
        if self.calculate_fscore(node) < c_min:
            c_node = node
            c_min = self.calculate_fscore(node)
...
def distance(self, node_1, node_2):
    ...
    x1 = self.map.intersections[node_1][0] - self.map.intersections[node_2][0]
    x2 = self.map.intersections[node_1][1] - self.map.intersections[node_2][1]
...
def get_tentative_gScore(self, current, neighbor):
    ...
    result = self.get_gScore(current)+self.distance(current,neighbor)
...
# **ANSWER**:
# 
# I would say is an algorithm that search the shortest path from the origin point to a destination point.
# 
...
# **ANSWER**:
# 
# I think that the difference between  A-Star search algorithm and the Uniform cost search is that the A-Star search algorithm uses heuristics approach.
# The different with the Best First Search algorithm and the A-Star search algorithm is that Best First Search is very fast and inexpensive but sometimes doesn't give you the best path, the A-Star search algorithm takes into consideration the cost of getting to a particular state

# ---
# What is a heuristic?
# 
# **ANSWER**:
# Heuristic is a method that is used to find a solution of a problem, when finding an optimal solution is impractical using other methods. The intention of the method is to speed up the process of reaching a satisfactory solution

# ---
# What is a consistent heuristic?
# 
# **ANSWER**:
# I think that is the sum of the cost of the current node plus the estimation cost from the next node and the result should be equal or less than the estimation cost from the current node to the destination goal.

# ---
# What is a admissible heuristic? 
# 
# **ANSWER**:
# Admissible is when the estimation cost is not greater than the actual cost, from the most recent or current node to the destination. 
