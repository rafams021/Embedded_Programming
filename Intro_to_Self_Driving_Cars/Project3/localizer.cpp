...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	// Initiate my implementation of the project.

	int height = grid.size();
	int width = grid[0].size();
	int area = height * width;
	float belief_per_cell = 1 / (float) area;
	vector <float> row;

	for(int i = 0; i < height; i++){
		// Initiate with clear row every loop.
		row.clear();
		for(int j = 0; j < width; j++){
			// Append belief_per_cell to row vector.
			row.push_back(belief_per_cell);
		}
		// Append row vector to newGrid matrix.
	...
	float p_miss) 
{
	...
	// Initiate my implementation if the project.

	int height = grid.size();
  	int width = grid[0].size();
	float total = 0.0;
	float new_belief = 0.0;
	float result = 0.0;
	vector <float> row;
